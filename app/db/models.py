...
from sqlalchemy import Column, Integer, String, DateTime, Boolean, Text, BigInteger, ForeignKey, UniqueConstraint
from sqlalchemy.sql import func

class Room(Base):
    __tablename__ = "rooms"

    id = Column(BigInteger, primary_key=True, autoincrement=True, index=True, comment='直播间id')
    room_type = Column(Integer, nullable=False, default=1, comment='房间类型：1-直播，2-会议')
    ...
    last_live_time = Column(DateTime(timezone=True), nullable=True, comment='最后直播/会议时间')

class RoomSessionMapping(Base):
    __tablename__ = "room_session_mappings"

    id = Column(BigInteger, primary_key=True, autoincrement=True, index=True, comment='映射ID')
    room_id = Column(BigInteger, ForeignKey('rooms.id'), nullable=False, comment='房间ID')
    session_id = Column(String(255), nullable=False, unique=True, comment='Cloudflare Calls session ID')
...
class SessionTrackMapping(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True, index=True, comment='映射ID')
    session_id = Column(BigInteger, ForeignKey('room_session_mappings.id'), nullable=False, comment='会话ID')
